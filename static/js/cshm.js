// cshm.js — Continuous Shape Measures (CShM)
//
// Ported from https://github.com/radi0sus/advanced_cshm-cc (static/js/cshm.js),
// which itself follows the same logic as the Python `cshm-cc` reference tool.
// Wrapped as a self-contained `CShM` namespace so it can sit next to Chem/Tables
// without leaking helper names (svd3, dot3, ...) into the global scope.
//
// Public API:
//   CShM.IDEAL_SHAPES            — reference polyhedra, keyed by shape label
//   CShM.calcCShM(central, lig)  — { shapeLabel: cshmValue, ... } for lig.length in 2..6
//
...
//
// Ideal reference coordinates are based on cosymlib-style ideal structures
// (see the advanced_cshm-cc README for the exact source), each containing
// CN ligand vertices + 1 central-atom vertex at the origin.
...
const CShM = (function () {
  ...
  // ── Polyhedral volume (convex hull of central atom + ligands) ────────────────
  //
  // Ported from https://github.com/radi0sus/advanced_cshm-cc
  // (static/js/geometry.js). Small brute-force convex-hull-by-facets routine,
  // fine for CN 2-6 point counts (<=7 points -> <=35 candidate triples).
  // Uses its own geom*() vector helpers (distinct names from the dot3/cross3/...
  // helpers above) to avoid any accidental name reuse within this module.
