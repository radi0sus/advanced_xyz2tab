...
const Tables = {
    ...
    // --- Plane distances (atoms to their own plane) ---
    renderPlaneDistances(container, planeResult, atoms, planeLabel) {
        if (!planeResult || atoms.length < 3) {
            container.innerHTML = '';
            return;
        }

        const { distances, rmsd } = planeResult;

        let html = `<div class="table-label">${planeLabel} — distances to plane (Å)</div>`;

        html += '<table class="data-table"><thead><tr>'
            + '<th>Atom</th><th>Distance (Å)</th>'
            + '</tr></thead><tbody>';

        atoms.forEach((atom, idx) => {
            html += `<tr><td>${atom.label}</td><td>${distances[idx].toFixed(4)}</td></tr>`;
        });

        html += '</tbody></table>';
        html += `<div class="result-box">RMSD: <span class="result-value">${rmsd.toFixed(4)} Å</span></div>`;

        container.innerHTML = html;
    },

    // --- Distances of plane2 atoms to plane1 ---
    renderPlane2ToPlane1(container, plane1Result, plane2Atoms) {
        if (!plane1Result || !plane2Atoms || plane2Atoms.length === 0) {
            container.innerHTML = '';
            return;
        }

        const { normal, centroid } = plane1Result;

        let html = '<div class="table-label">Plane 2 atoms — distances to Plane 1 (Å)</div>';

        html += '<table class="data-table"><thead><tr>'
            + '<th>Atom</th><th>Distance (Å)</th>'
            + '</tr></thead><tbody>';

        for (const atom of plane2Atoms) {
            const d =
                normal.x * (atom.x - centroid.x) +
                normal.y * (atom.y - centroid.y) +
                normal.z * (atom.z - centroid.z);

            html += `<tr><td>${atom.label}</td><td>${d.toFixed(4)}</td></tr>`;
        }
    ...
    renderPlaneAngle(container, angle) {
        if (angle === null) {
            container.innerHTML = '';
            return;
        }
        ...
        container.innerHTML = `
            <div class="result-box" style="margin-top:8px">
                <div class="table-label" style="margin-top:0">Plane 1 / Plane 2</div>
                Dihedral angle between planes:
                <span class="result-value" style="font-size:16px"> ${angle.toFixed(3)}°</span>
            </div>`;
    },
    ...
    toMarkdown(
        ...
        const lines = [];
        const { formula, fw, natoms, elCount, massFractions } = parsed;
        ...
        for (const [el, cnt] of Object.entries(elCount)) {
            const aw = Parser.atomicWeights[el] || 0;
            lines.push(`| ${el} | ${cnt} | ${aw.toFixed(3)} | ${(massFractions[el] || 0).toFixed(2)} |`);
        }
        ...
        if (bonds.length > 0) {
            ...
            bonds.forEach((b, i) => {
                lines.push(`| ${i + 1} | ${b.labelI} | ${b.labelJ} | ${b.dist.toFixed(4)} | ${b.manual ? 'manual' : 'auto'} |`);
            });
            ...
            lines.push(`**n:** ${s.n} | **Min:** ${s.min.toFixed(4)} | **Max:** ${s.max.toFixed(4)} | **Mean:** ${s.mean.toFixed(4)} | **Std:** ${s.std.toFixed(4)}`);
            lines.push('');
        }
        ...
        if (angles.length > 0) {
            ...
            angles.forEach((a, i) => {
                lines.push(`| ${i + 1} | ${a.labelA} | ${a.labelB} | ${a.labelC} | ${a.angle.toFixed(3)} |`);
            });
            ...
            lines.push(`**n:** ${s.n} | **Min:** ${s.min.toFixed(3)} | **Max:** ${s.max.toFixed(3)} | **Mean:** ${s.mean.toFixed(3)} | **Std:** ${s.std.toFixed(3)}`);
            lines.push('');
        }

        const writePlane = (label, planeAtoms, planeResult) => {
            if (!planeResult || planeAtoms.length < 3) return;

            lines.push(`## ${label}`);
            lines.push('');
            lines.push(`**Atoms:** ${planeAtoms.map(a => a.label).join(', ')}  `);
            lines.push(`**RMSD:** ${planeResult.rmsd.toFixed(4)} Å  `);

            const n = planeResult.normal;

            lines.push(`**Normal vector:** (${n.x.toFixed(4)}, ${n.y.toFixed(4)}, ${n.z.toFixed(4)})`);
            lines.push('');
            lines.push('| Atom | Distance to plane (Å) |');
            lines.push('|------|-----------------------|');

            planeAtoms.forEach((a, i) => {
                lines.push(`| ${a.label} | ${planeResult.distances[i].toFixed(4)} |`);
            });

            lines.push('');
        };

        writePlane('Plane 1', plane1Atoms, plane1Result);
        writePlane('Plane 2', plane2Atoms, plane2Result);

        // Distances of plane2 atoms to plane1
        if (plane1Result && plane2Atoms && plane2Atoms.length > 0) {
            lines.push('## Plane 2 atoms — distances to Plane 1');
            lines.push('');
            lines.push('| Atom | Distance to Plane 1 (Å) |');
            lines.push('|------|-------------------------|');
        ...
        if (planeAngle !== null && plane1Result && plane2Result) {
            lines.push(`**Dihedral angle between planes:** ${planeAngle.toFixed(3)}°`);
            lines.push('');
        }

        if (dihedralAngle !== null && dihedralAtoms.length === 4) {
            lines.push('## Dihedral Angle');
            lines.push('');
